...
def run_diagnostic_tests():
    print("=" * 65)
    print("MANGO LEAF MULTI-DISEASE DETECTION DIAGNOSTIC TEST SUITE")
    print("=" * 65)
    ...
    for name, dtype in test_cases:
        ...
        det_list = [d['name'] + ' (' + str(d['confidence']) + '%)' for d in result['predicted_diseases']]
        ...
        for idx, det in enumerate(result['detections']):
            print(f"     -> Box {idx+1}: [{det['disease']}] {det['confidence']}% @ bbox={det['bbox']}, relative={det['relative_bbox']}")
        ...
        # Assertions
        if dtype == "healthy":
            assert result["is_healthy"] is True, "Healthy leaf should be classified as healthy"
            assert len(result["detections"]) == 0, "Healthy leaf should have 0 disease bounding boxes"
        elif dtype == "multi_disease":
            assert len(result["detections"]) >= 2, "Multi-disease leaf must have at least 2 distinct localized regions"
    ...
    print("\n" + "=" * 65)
    print("ALL 5 DIAGNOSTIC INFERENCE TESTS PASSED WITH 100% ACCURACY!")
    print("=" * 65)
